...
class ZipPlaceSourceTests(unittest.TestCase):
    ...
    def parse_postnummer_closest_to(self):
        result = _parse_postnummer_closest_to(GEONORGE_RESPONSE, self.latitude, self.longitude)
        self.assertIsInstance(result, unicode)
        self.assertEqual(len(result), 4)

    def parse_postnummer_closest_to_resultTypeIsExpected(self):
        result = _parse_postnummer_closest_to(GEONORGE_RESPONSE, self.latitude, self.longitude)
        self.assertIsInstance(result, unicode)

    def parse_postnummer_closest_to_resultIsNone(self):
