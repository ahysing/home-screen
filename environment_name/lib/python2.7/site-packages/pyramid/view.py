...
class AppendSlashNotFoundViewFactory(object):
    ...
    Not Found view.  However, if you wish to use a *custom* not_found
    ...
    custom not_found view as the first argument to its constructor.
    For instance:
...
class notfound_view_config(object):
    ...
    argument restricts the set of circumstances under which this not_found
    view will be invoked.

    If ``append_slash`` is ``True``, when the Not Found View is invoked, and
    the current path info does not end in a slash, the not_found logic will
...
class forbidden_view_config(object):
    ...
    the set of circumstances under which this not_found view will be invoked.
